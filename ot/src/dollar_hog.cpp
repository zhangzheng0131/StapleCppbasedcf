...
static float* fhog(float* I,
                   int height,int width,int channel,
                   int *h,int *w,int *d,int binSize = 4,
                   int nOrients = 9,float clip=0.2f,
                   bool crop = false);

static void change_format(float *des, float *source,
                          int height, int width, int channel);
...
void grad1( float *I, float *Gx, float *Gy, int h, int w, int x ) {
  int y, y1; float *Ip, *In, r; M128 *_Ip, *_In, *_G, _r;
...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  int x, y, y1, c, h4, s; float *Gx, *Gy, *M2; M128 *_Gx, *_Gy, *_M2, _m;
  float *acost = acosTable(), acMult=10000.0f;
  // allocate memory for storing one column of output (padded so h4%4==0)
  h4=(h%4==0) ? h : h-(h%4)+4; s=d*h4*sizeof(float);
  M2=(float*) alMalloc(s,16); _M2=(M128*) M2;
  Gx=(float*) alMalloc(s,16); _Gx=(M128*) Gx;
  Gy=(float*) alMalloc(s,16); _Gy=(M128*) Gy;
...
void gradMagNorm( float *M, float *S, int h, int w, float norm ) {
  M128 *_M, *_S, _norm; int i=0, n=h*w, n4=n/4;
  _S = (M128*) S; _M = (M128*) M; _norm = SET(norm);
  ...
  int nb, int n, float norm, int nOrients, bool full, bool interpolate )
{
  ...
  M128i _o0, _o1, *_O0, *_O1; M128 _o, _od, _m, *_M0, *_M1;
  // define useful constants
  const float oMult=(float)nOrients/(full?2*PI:PI); const int oMax=nOrients*nb;
  const M128 _norm=SET(norm), _oMult=SET(oMult), _nbf=SET((float)nb);
  const M128i _oMax=SET(oMax), _nb=SET(nb);
  // perform the majority of the work with sse
  _O0=(M128i*) O0; _O1=(M128i*) O1; _M0=(M128*) M0; _M1=(M128*) M1;
  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    if( softBin<0 && softBin%2==0 ) {
      ...
      float ms[4], xyd, yb, xd, yd; M128 _m, _m0, _m1;
...
cv::Mat fhog(const cv::Mat& input, int binSize, int nOrients, float clip, bool crop){
    ...
    int count = HEIGHT*WIDTH*DEPTH;
    float *II = new float[count];
