...
/*    float scale_exp[nScales];
  ...
  //  shift(interp_scale_exp,interp_scale_exp_shift,[0 -floor((nScalesInterp-1)/2)]);
   ...
   // printf("\n");
    ...
    for(int i=0;i<nScalesInterp;i++)
    {
        pinterpScaleFactors[i]=pow(scale_step,interp_scale_exp_shift1[i]);
    ...
    pmin_scale_factor=pow(scale_step,ceil(log(((5/psz[0])>(5/psz[1])?(5/psz[0]):(5/psz[1]))))/log(scale_step));
    pmax_scale_factor=pow(scale_step,floor(log(roi.h/target_sz[0]<roi.w/target_sz[1]?(roi.h/target_sz[0]):(roi.w/target_sz[1]))));
    ps_num_compressed_dim=nScales;
    //printf("m_target_size 333333: %f , %f\n",m_cfs[idx].target_size[0],m_cfs[idx].target_size[1]);

    float  ys[nScales];
    cv::Mat ys1=cv::Mat(cv::Size(nScales,1),CV_32FC1);
    float *pys= (float *)(ys1.data);
    for(int i=0;i<nScales; i++)
    {
        pys[0]=exp((-0.5)*(pow(scale_exp_shift[i],2))/pow(scale_sigma,2));
        pys++;
    }
    //m_cfs[idx].ysf=FFTTools::fftd1d(ys1,2,true);
    //m_cfs[idx].ysf=cvFFT(ys1,2,true);


   float * pysf= (float *)(m_cfs[idx].ysf.data);
    float a[17]={2.6633,2.4657,1.9565,1.3306,0.7756,0.3875,0.1662,0.0621,0.0243,0.0243,0.0621,0.1332,0.3875,0.7756,1.3306,1.9565,2.4657};
    for(int i=0;i<m_cfs[idx].ysf.cols;i++)
    {
        pysf[0]=a[i];
        //printf("%f\n",pysf[0]);
        pysf++;
...
{
    Rect_T roi;
    //printf("m_target_size 4444444: %d , %d\n",m_cfs[idx].target_size[0],m_cfs[idx].target_size[1]);
...
/*#ifdef ENABLE_SUB_PEAK
    ...
    //if(pf.x> resCF.cols *0.5f){pf.x=pf.x-resCF.cols;}
    //if(pf.y> resCF.rows * 0.5f){pf.y =pf.y-resCF.rows;}
...
cv::Mat StapleTracker::getSubWin(int idx)
{
    ...
    //printf("m_target_size 555555: %d , %d\n",m_cfs[idx].target_size[0],m_cfs[idx].target_size[1]);
   ...
   /* for(int i=0;i<ztemp.rows;i++)
    {
        for(int j=0;j<ztemp.cols;j++)
        {
            for(int n=0;n<ztemp.channels();n++)
            {
                if(n==0) {
                  // printf("%d\t", z.data[i * roiGrayzz.cols + j]);
                    printf("%d\t", ztemp.data[i * ztemp.cols *ztemp.channels()+ j*ztemp.channels() + n]);
                    //printf("%d\t", pzz[0]);
                }
                if(j==ztemp.cols-1) printf("\n");
               // pzz++;
                                ...
                                float lr, bool isInit,double BACF_lr)
{
    ...
    float * py= (float *)(m_cfs[idx].y.data);

   /* for(int i = 0; i < m_cfs[idx].y.rows;i++)
    {
        for(int j=0;j<m_cfs[idx].y.cols;j++)
        {
            printf("yf: %f + %f i \t",py[0],py[1]);
            py+=2;
        }
        printf("\n");
    }
...
//
      ...
      //  padded.setTo(cv::Scalar::all(0));
       ...
       /* for(int iz=0;iz<xtemp.rows;iz++)
        ...
        //MatExpr resF11= cv::Mat::zeros(m_cfs[idx].df.rows, m_cfs[idx].df.cols,
         //                              CV_64FC2);
...
int StapleTracker::detectScaleCF(int idx, float &conf)
{
   ...
   // float bestScale = m_scale_factors[pi.x];
    ...
    //printf("best scale detected: %f \n",bestScale);
    //printf("scale detected: %f \n", m_cfs[idx].scale_adapt);
   ...
   // m_cfs[idx].target_size[1] = round(tz_h);
    ...
    //printf("currentScaleFactor: %f, m_target_size1 %f + %f \n",m_cfs[idx].currentScaleFactor,m_cfs[idx].target_size1[0],m_cfs[idx].target_size1[1]);
    //printf("m_target_size   1111111111001: %d , %d\n",m_cfs[idx].target_size[0],m_cfs[idx].target_size[1]);
